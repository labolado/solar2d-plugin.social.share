...
public class showPopup implements com.naef.jnlua.NamedJavaFunction
{
	...
	private String localAbsoluteTmpFilePath;
	...
	private boolean copyFileToTmpDir(String src, String prefix) {
		...
		try {
			...
			File dstFile = File.createTempFile(prefix, getExtension(src), external);
			localAbsoluteTmpFilePath = dstFile.getAbsolutePath();

			inputStream = new java.io.FileInputStream(srcFile);
			outputStream = new java.io.FileOutputStream(dstFile);
			...
			Log.d("Corona.showPopup", "copyFileToTmpDir:" + ex.getMessage());
			ex.printStackTrace();
		} finally {
			if (inputStream != null) {
			...
			if (outputStream != null) {
				try {outputStream.close();} catch (Exception ex) {ex.printStackTrace();}
			}
		}
		return result;
	}
	...
	public int invoke( final LuaState luaState )
	{
		...
		try
		{
			...
			if ( 1 == images.size() )
			{
				...
				// Set the image uri
				// File localFile = new File(images.get(0));
				...
				String type = fileServices.getMimeTypeFrom( imageUri );
				String authority = coronaApplication.getPackageName() + ".my_files";
				imageUri = androidx.core.content.FileProvider.getUriForFile( coronaApplication, authority, new File(images.get(0)) );

				// boolean ok = copyFileToTmpDir(images.get(0), "shareTmp");
				// if (ok) {
				// 	// imageUri = Uri.parse(localAbsoluteTmpFilePath);
				// 	imageUri = FileContentProvider.createContentUriForFile( coronaApplication, localAbsoluteTmpFilePath );
				// }
				// Log.d("Corona.showPopup", imageUri.toString());
				// Log.d("Corona.showPopup", imageUri.getEncodedPath());
				// Log.d("Corona.showPopup", type);
				// Uri imageUri = Uri.fromFile(new java.io.File(images.get(0)));
                // Uri imageUri = Uri.parse(MediaStore.Images.Media.in)
			...
			else if ( images.size() > 1 )
			{
			    ...
			    for ( int i = 0; i < images.size(); i ++ )
			    {
					...
					imageUri = androidx.core.content.FileProvider.getUriForFile( coronaApplication, authority, new File(images.get(i)) );
